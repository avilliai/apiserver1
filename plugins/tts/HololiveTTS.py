...
class HoliveTTS:
    ...
    def __new__(cls) -> "HoliveTTS":
        if cls._instance is None:
            ...
            cls._instance._session: Optional[httpx.AsyncClient] = None
    ...
    async def open(self) -> None:
        if self._session is None or self._session.is_closed:
            # 加入 timeout 限制，Gradio 生成长文本可能会超过默认的 5 秒
    ...
    async def _poll(self, session_hash: str) -> str:
        ...
        # httpx 请求 SSE 推荐使用 stream
        async with self._session.stream("GET", url, headers=HEADERS, timeout=120.0) as resp:
            resp.raise_for_status()
    ...
    async def _download(self, audio_url: str) -> bytes:
        resp = await self._session.get(audio_url, headers=HEADERS)
        resp.raise_for_status()
        # httpx 获取二进制内容使用 .content
        return resp.content
